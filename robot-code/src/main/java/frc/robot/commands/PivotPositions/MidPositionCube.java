...
public class MidPositionCube extends CommandBase {
  /** Creates a new MidPosition. */
  private Arm m_arm;
  public MidPositionCube(Arm arm) {
    m_arm = arm;
    addRequirements(arm);
  ...
  public void execute() {
    //Rotate
    if (m_arm.getPivotEncoder() >= Constants.EncoderConstants.MidPositionCube) {
      m_arm.rotatePivot(-0.5);
    } else if (m_arm.getPivotEncoder() <= Constants.EncoderConstants.MidPositionCube) {
      m_arm.rotatePivot(0.5);
    }
  }
